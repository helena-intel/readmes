...
for best practices. In particular: make sure that the NPU driver is updated, use symmetric INT4 quantization to export
the model, for models > 4GB, use `--group-size -1`, for smaller models `--group-size 128` . Use OpenVINO 2024.6. Note that
only greedy search is supported (use `do_sample=False` in `pipe.generate()`). Definitely use model caching.
